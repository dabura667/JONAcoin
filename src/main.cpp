...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // JONAcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "JONAcoin Signed Message:\n";
...
{
    // JONAcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 525600 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 525600); // JONAcoin: 525.6k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // JONAcoin: 1 days
static const int64 nTargetSpacing = 1 * 60; // JONAcoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // JONAcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // JONAcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // JONAcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 2014/2/15 Spying by N.S.A. Ally Entangled U.S. Law Firm";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1392555472;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2088545493;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // JONAcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JONAcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JONAcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JONAcoinMiner(CWallet *pwallet)
{
    printf("JONAcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("JONAcoin-miner");
    ...
    try { loop {
        ...
        printf("Running JONAcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JONAcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JONAcoinMiner, pwallet));
}
